...
import LangCode from "../Records/LangCode";
import SWFBitReader from "../SWFBitReader";
import SWFTags from "./SWFTags";

export default class DefineFontInfo2Tag implements ITag {
    private _size: number
    private _data: Buffer

    private constructor(data: Buffer, size: number, fontID: number, fontNameLen: number, fontName: string, ffSmallText: boolean, ffShiftJIS: boolean, ffANSI: boolean, ffItalic: boolean, ffBold: boolean, ffWideCodes: boolean, langCode: LangCode, codeTable: number[]) {
        this._data = data
        this._size = size
        this.FontID = fontID
    ...
    get TagCode(): SWFTags {
        return SWFTags.DefineFontInfo2
    }
    get Size(): number {
        return this._size
    }
    ...
    public static ReadData(br: SWFBitReader, nGlyphs: number): DefineFontInfo2Tag {
        ...
        let retLanguageCode: LangCode = br.ReadByte()
